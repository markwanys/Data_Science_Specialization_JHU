...
Reference for dataset and literature:
...
for (name in names(training_set)){
  if ((class(training_set[,name])=='numeric')|(name=='classe')) {
    num_col <- append(num_col,name)
  }
}

training_set <- training_set[num_col]
testing_set <- testing_set[num_col]
testing <- testing[num_col[!num_col %in% "classe"]]
dim(training_set)
```
...
corrs <- cor(subset(training_set,select=-c(classe)))
corrplot(corrs, order = "FPC", method = "color", type = "lower", tl.cex = 0.8, tl.col = rgb(0, 0, 0))
```
...
names(training_set)[which(names(training_set)=='training_set$classe')] <- 'classe'
dim(training_set)

testing_set_PC <- predict(preProc,subset(testing_set,select=-c(classe)))
testing_set <- cbind(testing_set_PC,testing_set$classe)
names(testing_set)[which(names(testing_set)=='testing_set$classe')] <- 'classe'
...
preObj <- preProcess(testing,method=c("center","scale"))
testing_std <- predict(preObj,testing)
...
Classifier    |Accuracy (Out-of-Sample estimate)
-------------|---------
...
```{r test}
data.frame(case=c(1:dim(testing)[1]),Prediction=predict(modelFit_rf,testing_PC))
```
